...
import FtpClientPanel from './ftp-client-panel'
import ftpClientScxml from './ftp-client.scxml'

export { ftpClientScxml }

export default class QtFtpClientExample extends React.Component {
  constructor(props) {
    super(props)
    this.state = { log: [], commands: [] }
  }

  componentDidMount() {
    scxml.documentStringToModel(null, ftpClientScxml, (err, model) => {
      if (err) throw err
  ...
  customSend(event) {
    ...
    } else if (event.name === 'submit.cmd') {
      this.setState(({ commands }) => ({ commands: commands.concat(event.data).slice(-12) }))
    } else {
      this.sc.gen(event)
    }
  }
      ...
      <div className="row" style={{ minHeight: '560px' }}>
        <div className="col-md-8" style={{ marginBottom: '1rem' }}>
          <div style={{ height: '560px', width: '100%', position: 'relative', border: '1px solid #eee' }}>
            <SCHVIZ
              scxmlDocumentString={ftpClientScxml}
