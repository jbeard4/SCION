...
testSerializations.forEach(function(testSerialization){
  testGenAsync.forEach(function(testGenAsync){
    tests.forEach(function(test){
      if((testSerialization && test.skipSerializationTests) || (testGenAsync && test.skipAsyncTests)) return;
      testOptions.push({
...
module.exports = function(Statechart, options){

  var fixtures = {};
  var testsToIgnore = (options && options.testsToIgnore) || []; 

  testOptions.
    filter(function(test){ return testsToIgnore.indexOf(test.name) === -1}).
