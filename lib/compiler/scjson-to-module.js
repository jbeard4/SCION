    ...
    Promise.all(scriptPromises).then(function resolved(scripts) {
        ...
        this.rootScripts.forEach(function(rootScript){
          if(rootScript.$wrap){
            ...
            //TODO: determine if he has an http uri, file uri, or no protocol specified (defaults to file)
            //if http, treat as though it's an absolute path
            //if not http, assume file. determine if he is a relative or an absolute path
            //if he is an absolute path, then resolve him relative to scxml host doc
            //if he is a relative path, then resolve him relative to scxml host doc
            var scriptSrc = rootScript.src ? path.resolve(path.dirname(this.docUrl),rootScript.src) : this.docUrl;
            parseJsCode(rootScript.content, {$line : 0, $column : 0}, scriptSrc, injectionNode, false)
          }
          injectionNode.add('\n');
        },this)
...
ModuleBuilder.prototype.handleExternalActionScript = function(action) {
    ...
    if (!this.externalActionScripts.has(action.src)) {
        ...
        action.$wrap = function(body) {
            //TODO: here too. we also want to make action.src an absolute path, if he is relative.
            var scriptSrc = path.resolve(path.dirname(this.docUrl),action.src);
            return generateFnDeclaration(fnName,body,{$line : 0, $column:0},scriptSrc,false,true);
        }.bind(this);
