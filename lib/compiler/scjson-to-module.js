...
function startTraversal(rootState, moduleType){
    ...
    return {
        moduleString: generateModule(rootState,moduleType),
        name: rootState.name
    };
}
...
var actionTags = {
    ...
    "custom": function (action) {
        ...
        for(var propertyName in action) {
           if(propertyName.indexOf('expr') !== -1) {
                action[propertyName] = processCustomTagExpression(action, propertyName);
           }
        }

        return "postMessage(" + JSON.stringify(customTagConfig, null, 4).replace(REFERENCE_MARKER_RE,"$1" + FN_ARGS) + ");"
    }
};

function processCustomTagExpression (container, attr) {
    var fnName = generateAttributeExpression(container, attr);

    return markAsReference(fnName);
}
// function processCustomTagExpression(container,attr){
