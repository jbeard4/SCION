...
const tagRegistry = require('../runtime/tagRegistry').registry;
...
function parseJsCode(fnBody, action, docUrl, node){
...
function generateFnDeclaration(fnName,fnBody,actionOrInvoke,docUrl,isExpression,parseGeneratedCodeForSourceMap){
    ...
    node.add(
      new module.exports.SourceNode(
        actionOrInvoke.$closeLine + 1,
        actionOrInvoke.$closeColumn,
        docUrl,
        '\n};\n'
      )
    );
...
ModuleBuilder.prototype.generateFnCall = function(fnName, ...args){
...
ModuleBuilder.prototype.generateInvokeFunction = function(invoke, parentState) {
    ...
    const processAttr = this.processSendOrInvokeAttr.bind(this, invoke);
      ...
      this.invoke({
        ...
        ${
          invoke.content && invoke.content.expr ? 
            `"content" : ${this.generateFnCall(this.generateAttributeExpression(invoke.content, 'expr', invoke.$type))},` : 
            ''
        }
        ${
          (invoke.namelist || (invoke.params && invoke.params.length)) ? 
            `"params" : ${this.generateFnCall(actionWithNamelistAndParamsToProps(invoke, this))},` : 
            ''
        }
...
ModuleBuilder.prototype.generateActionFunction = function(action) {
    ...
    const m = action.$type.match(stripNsPrefixRe);
    const ns = m[1] || constants.SCXMLNS;
    const tagName = m[2];
    ...
    if(delcarationsToInit.length){
      var fnBody = delcarationsToInit.map(function(data){
              return 'if(typeof ' + data.id + ' === "undefined")  ' + data.id + ' = ' + 
                builder.generateFnCall(data.expr ? 
...
function generateFactoryFunctionWrapper(o, constructorFunctionName, rootState){
...
ModuleBuilder.prototype.generateModule = function(){
    ...
    var o = {
                ...
                sendString : (this.documentHasSendAction ? getDelayInMsFnStr + '\n' : ''),
...
ModuleBuilder.prototype.processSendOrInvokeAttr = function(container, attr, parentState){
    ...
    if(attr === 'id'){
      let arr = this[container.$type === 'invoke' ? 'invokeIdAccumulator' : 'sendIdAccumulator'];
    ...
    if(attr === 'idlocation'){
      ...
      fnName = this.generateAttributeExpression(container, attr, container.$type);
      return this.generateFnCall(fnName);
    }else if(container[exprName]){
        fnName = this.generateAttributeExpression(container, exprName, container.$type);
        return this.generateFnCall(fnName);
...
const getDelayInMsFnStr = `function getDelayInMs(delayString){
