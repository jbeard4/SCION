      ...
      try {
        ...
        try {
          ...
          parser.onend = function () {
            //do the thing   
            fnBody = `this.parseXmlStringAsDOM(${JSON.stringify(exprObj.content)})`;
          };
          
          parser.write(exprObj.content).close();
        } catch (e) {
          //otherwise, assume he should be interpreted as a string
          fnBody = JSON.stringify(normalizeWhitespace(exprObj.content))
        }
      }
    } else {
      fnBody = exprObj.expr
