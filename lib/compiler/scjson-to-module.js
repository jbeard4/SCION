...
var moduleAnalyzer = require('./static-analysis/module-analyzer');
...
function generateFactoryFunctionWrapper(o){
    ...
    return '(function(_x,_sessionid,_name,_ioprocessors,In){\n' + 
...
function startTraversal(rootState, moduleType){
    ...
    var moduleJsString = generateModule(rootState,moduleType);

    return analyzeModule(moduleJsString);
}

//Run compiled javascript code through static analyzing for
//checking illegal expressions and syntax errors
function analyzeModule (jsString) {
    return moduleAnalyzer.analyze(jsString);
}
...
function go(){
    var moduleString = startTraversal(JSON.parse(jsonString),moduleType);
