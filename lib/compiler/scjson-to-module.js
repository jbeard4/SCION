      ...
      this.invoke({
        ...
        ${
          invoke.expr || invoke.content ? 
            `"content" : ${generateFnCall(this.generateAttributeExpression(invoke, invoke.expr ? 'expr' : 'content'))},` : 
            ''
        }
...
ModuleBuilder.prototype.generateExpressionFunction = function(expressionType,exprObj,isContent){
    ...
    var fnDec = generateFnDeclaration(fnName,isContent ? JSON.stringify(exprObj.content) : exprObj.expr,exprObj,this.docUrl,true,true);
...
ModuleBuilder.prototype.generateAttributeExpression = function(attrContainer,attrName){
    ...
    return this.generateExpressionFunction(stripAttrNsPrefix(attrName), attrContainer[attrName], attrName === 'content');
}
                            ...
                            map(function(data){
                              return 'if(!' + data.id + ')  ' + data.id + ' = ' + 
                                generateFnCall(data.expr ? 
                                  this.generateExpressionFunction('data',  data.expr) : 
                                  this.generateExpressionFunction('data',  data.content, true) 
                                ) + ';\n';
                            }, builder).join('') +
                '        ' + EARLY_BINDING_DATAMODEL_GUARD + ' = true; ' + '\n' +
                '    }\n' +
...
var actionTags = {
    ...
    "assign" : function(action, builder){
        return action.location.expr + " = " + 
          generateFnCall(builder.generateAttributeExpression(action, action.expr ? 'expr' : 'content')) + ";";
    },
...
function processSendOrInvokeAttr(container, builder, attr, parentState){
    ...
    if(attr === 'idlocation'){
      ...
      container[attr].expr = container[attr].expr + '=' + generateFnCall(container.$type === 'invoke' ? GENERATE_INVOKEID_FN_NAME : GENERATE_SENDID_FN_NAME, JSON.stringify(parentState.id));   //TODO: get parent state id for invoke
