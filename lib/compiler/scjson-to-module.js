...
ModuleBuilder.prototype.generateInvokeFunction = function(invoke) {
    ...
    var invokeId;
    if(invoke.id){
      invokeId = processAttr('id');
    } else if(invoke.idlocation){
      invokeId = processAttr('idlocation');
    } else{
      invokeId = 'null';
    }
    ...
    var fnBody = `
      this.invoke({
        "type" : ${processAttr('type')} || ${JSON.stringify(DEFAULT_INVOKE_TYPE)},
        "src" : ${processAttr('src')},
        "id" : ${invokeId},
        "content" : ${JSON.stringify(invoke.content)},
        "params" : ${actionWithNamelistAndParamsToProps(invoke, this)},
...
function processSendOrInvokeAttr(container, builder, attr){
    ...
    if(attr === 'id'){
      builder[container.$type === 'invoke' ? 'invokeIdAccumulator' : 'sendIdAccumulator'].push(container[attr]);
    }
    ...
    if(attr === 'idlocation'){
      ...
      container[attr].expr = container[attr].expr + '=' + generateFnCall(container.$type === 'invoke' ? GENERATE_INVOKEID_FN_NAME : GENERATE_SENDID_FN_NAME);
...
function generateIdlocationGenerator(type, fnName, idAccumulator){
    return `
      ...
      function ${fnName}(){
        var id;
        do{
          id = "$scion.${type}id" + $${type}IdCounter++; //make sure we dont clobber an existing sendid or invokeid
