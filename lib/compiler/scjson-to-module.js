...
function ModuleBuilder(rootState, options) {
...
function startTraversal(rootState, options){
    if (!options) {
        options = {};
    }

    var moduleBuilder = new ModuleBuilder(rootState, options);
...
/**
 * Handle an externally referenced script within an executable content block
...
ModuleBuilder.prototype.handleExternalActionScript = function(action) {
    var fnName = generateFnName(SCRIPT_SRC_FN_PREFIX, action);
...
function go(){
    startTraversal(JSON.parse(jsonString), {debug: true}).then(
