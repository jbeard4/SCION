...
ModuleBuilder.prototype.getInvokeId = function(invoke, parentState){
    var invokeId;
    if(invoke.id){
...
ModuleBuilder.prototype.generateInvokeFunction = function(invoke, parentState) {
    ...
    let invokeId = this.getInvokeId(invoke, parentState);
    ...
    var fnBody = `
      ...
      this.invoke({
        ...
        ${
          invoke.expr ? 
            `"content" : ${generateFnCall(this.generateAttributeExpression(invoke, 'expr', invoke.$type))},` : 
            ''
        }
        ${
...
function generateFactoryFunctionWrapper(o, invokeConstructor, rootState, options){
    
    var root = new SourceNode(null,null,null,[
              //o.headerString + '\n' ,
              'function ' + getConstructorFunctionName(invokeConstructor || rootState) + '(_x,_sessionid,_ioprocessors,In){\n' ,
...
ModuleBuilder.prototype.generateModule = function(){
    ...
    var s = generateFactoryFunctionWrapper(o, this.invokeConstructor, rootState, options);
...
SCJsonRawModule.prototype.prepareModuleString = function(cb, options){
  return Promise.all(this.invokeConstructors.map( constructor => constructor.prepareModuleRootNode() )).then( function(rootNodes){
    ...
    if(isCommonjs(options)) {
      ...
      rootNode.add('   return ' + getConstructorFunctionName(rootState) + ';\n');
      rootNode.add('})();');
    }
...
function InvokeConstructor(rootScripts, datamodel, moduleSourceNode){
