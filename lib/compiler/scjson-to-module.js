...
function generateFnName(actionType,action){
    var tagName = stripAttrNsPrefix(actionType);
    return '$' + tagName + '_line_' + action.$line + '_column_' + action.$column;
}
...
function generateFnDeclaration(fnName,fnBody,action){
    ...
    if(typeof fnBody === 'undefined' || fnName === 'undefined'){
      throw new Error('Undefined function template');
    }
...
function generateActionFunction(action){
    ...
    if(!actionTags[action.$type]){
      console.log('actionTags[action.$type]',action.$type,actionTags);
    ...
    return  datamodelAccumulator.length ? 
                ...
                '    if(!' + EARLY_BINDING_DATAMODEL_GUARD + '){\n' + 
                         ...
                         datamodelAccumulator.
                            ...
                            map(function(data){ debugger; var fn = generateExpressionFunction(data.$type,data.expr); return '        ' + data.id + ' = ' + generateFnCall(fn) + ';\n';}).join('') + 
                '        ' + EARLY_BINDING_DATAMODEL_GUARD + ' = true; ' + '\n' +
                '    }\n' +
                '}' : '';
}
...
function populateActionTags(customActionElements){
    //re-init actionTags
    actionTags = {
     "http://www.w3.org/2005/07/scxml" : scxmlActionTags
    };
    ...
    Object.keys(allNamespaces).forEach(function(namespace){
      Object.keys(allNamespaces[namespace]).forEach(function(tagName){
        actionTags['{' + namespace + '}' + tagName] = allNamespaces[namespace][tagName];
      });
    });
}

function startTraversal(rootState, context){
    var moduleType = context.moduleType || 'siaf';
    //console.log('actionTags',actionTags);

    populateActionTags(context.customActionElements || {});
...
//TODO: break this out so that it is registered via the same plugin interface
var scxmlActionTags = {
    ...
    "if" : function(action){
        ...
        for(var i = 0; i < childNodes.length; i++){
            ...
            if(child.$type === "{http://www.w3.org/2005/07/scxml}elseif" || child.$type === "{http://www.w3.org/2005/07/scxml}else"){
                break;
            }else{
        ...
        for(; i < childNodes.length; i++){
            ...
            if(child.$type === "{http://www.w3.org/2005/07/scxml}elseif"){

                s+= "}else if(" + generateFnCall(generateAttributeExpression(child,'cond'))  + "){\n";
            }else if(child.$type === "{http://www.w3.org/2005/07/scxml}else"){
                s += "}";
                break;
            }else{
                s += '    ' + generateFnCall(generateActionFunction(child)) + ';\n';
        ...
        for(; i < childNodes.length; i++){
            ...
            if(child.$type === "{http://www.w3.org/2005/07/scxml}else"){
                s+= "else{\n";
            }else{
                s += '    ' + generateFnCall(generateActionFunction(child)) + ';\n';
            }
...
function go(){
    var o = startTraversal(JSON.parse(jsonString),{moduleType : moduleType});
