...
ModuleBuilder.prototype.generateInvokeFunction = function(invoke) {
    if(printTrace) console.log('generateActionFunction',invoke);

    const invokeType = invoke.type || DEFAULT_INVOKE_TYPE;
    var fnName = generateFnName('invoke',invoke);
    var fnBody = invokeTypes[invokeType](invoke, this);
...
ModuleBuilder.prototype.generateExpressionFunction = function(expressionType,exprObj){
    ...
    var fnDec = generateFnDeclaration(fnName,exprObj.expr,exprObj,this.docUrl,true,true);
    ...
    (builder.datamodelAccumulator.length ?
                ...
                '    if(!' + EARLY_BINDING_DATAMODEL_GUARD + '){\n' +
                         ...
                         builder.datamodelAccumulator.
                            ...
                            map(function(data){return 'if(!' + data.id + ')  ' + data.id + ' = ' + generateFnCall(builder.generateExpressionFunction('data',data.expr)) + ';\n';}, builder).join('') +
                '        ' + EARLY_BINDING_DATAMODEL_GUARD + ' = true; ' + '\n' +
                '    }\n' +
                '}' : '');
}
...
ModuleBuilder.prototype.replaceActionCode = function(actionContainer,actionPropertyName, functionGenerator){
    ...
    if(actionContainer[actionPropertyName]){
        ...
        actionContainer[actionPropertyName] = actions.map(functionGenerator, this).map(markAsReference);
...
const invokeTypes = {
  "http://www.w3.org/TR/scxml/" : function(invoke, builder){
    ...
    if(invoke.content){
      //this becomes the template string
    }else if(invoke.src){
      var urlObj = urlM.parse(invoke.src);
      var protocol = urlObj.protocol;
      if(protocol === 'http:' || protocol === 'https:'){
        scxml.urlToModel(); 
      }else if(protocol === 'file:' || !protocol){
        //TODO: handle attribute lookup: srcexpr
        //TODO: break out this code into a new public API: scxml.invoke(src);
        return `
          var self = this;
          var params = ${actionWithNamelistAndParamsToProps(invoke, builder)};
          scxml.pathToModel(${JSON.stringify(path.isAbsolute(urlObj.pathname) ? urlObj.pathname : path.join(path.dirname(builder.docUrl),urlObj.pathname) )}, function(err, model){
            ...
            model.prepare(function(err, fnModel){
              var interpreter = new scxml.scion.Statechart(fnModel, {
                parentSession :  self, //TODO: pass through a reference to this intepreter
                params : params 
                //sessionid : //TODO: construct or generate a sessionid
              });
              interpreter.start();
            });
    ...
    "send" : function(action, builder){
        ...
        function constructSendEventData(action){
            ...
            if(action.content){
                ...
                //TODO: Factor out send/@contentexpr, replace with send/content/@expr
