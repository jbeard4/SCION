...
function getNormalizedNodeOrAttributeName(attr){
...
function copyNsAttrObj(o){
    ...
    Object.keys(o).forEach(function(k){
        var attr = o[k];
        r[getNormalizedNodeOrAttributeName(attr)] = attr.value; 
    });
    return r;
}
...
function transform(xmlString){
    ...
    function createActionJson(node){
        var action = merge(
                    {
                        ...
                        $type: getNormalizedNodeOrAttributeName(node)
                    },
        ...
        }else if(currentJson.$type === 'scxml' && action.$type === '{http://www.w3.org/2005/07/scxml}script'){
            //top-level script
    ...
    function createDataJson(node){
        currentJson = 
            merge({
                $line : parser.line,
                $column : parser.column,
                $type : '{http://www.w3.org/2005/07/scxml}data'
            },
            copyNsAttrObj(node.attributes));
        return currentJson; 
    }
    ...
    parser.onattribute = function (attr) {
        ...
        if( attr.name.match(/^.*expr$/) ||
            ...
            expressionAttributeCache[getNormalizedNodeOrAttributeName(attr)] = createExpression(attr.value);
        }
    };
