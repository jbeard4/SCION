...
var contentRE1 = /<content\s*>/g,
    contentRE2 = /<\/content\s*>/g;
function wrapContentInCDATA(xmlString){
  //most of the time we do not want to parse <content> as XML
  //so we transform the document so that contents of <content> tags are interpreted as CDATA
  return xmlString.
          replace(contentRE1,'$0<![CDATA[').
          replace(contentRE2,']]>$0');
  
} 

function transform(xmlString){
    xmlString = wrapContentInCDATA(xmlString);
    ...
    parser.ontext = function (t) {
        ...
        if(currentJson && currentJson.$type){
            ...
            }else if(currentJson.$type === 'send' && traversingInContentTag){
                currentJson.content = t;
            }else if(currentJson.$type === 'data' || currentJson.$type === 'assign'){
                currentJson.content = {
