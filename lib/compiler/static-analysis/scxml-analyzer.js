...
var openNodeTypes = {
    ...
    "foreach": function (node) {
        if(node.attributes.item) {
            ...
            validateJavascriptExpression(constructedExpression, parser.startTagPosition - 1, getClosingTagIndex('foreach'));
        }
    }
};
...
function getClosingTagIndex (currentTagName) {
    var closingTagString = "</" + currentTagName + ">";

    return parser.position + currentScxml.substring(parser.position).indexOf(closingTagString) + closingTagString.length;
}
