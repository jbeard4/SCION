...
var openNodeTypes = {
    ...
    "send": function (node) {
        if(node.attributes.type) {
            if(!eventProcessorTypes[node.attributes.type.value]) {
                //Check if type is supported

                var newError = {
                    message: 'Event I/O processor is not supported.',
                    start: parser.startTagPosition - 1,
                    end: parser.position,
                    line: parser.line,
                    col: parser.column,
                ...
                createError(newError);
            } else if(node.attributes.typeexpr) {
                //type and typeexpr can't be used at the same time

                var newError = {
                    message: '"type" and "typeexpr" can\'t be used at the same time',
                    start: parser.startTagPosition - 1,
                    end: parser.position,
                    line: parser.line,
                    col: parser.column,
            ...
            validateJavascriptExpression(node.attributes.typeexpr.value, parser.startTagPosition - 1, parser.position);
        }
    }
};
