...
var actionTags = {
	"data": function (node, cb) {
		validateJavascriptExpression(node.id, node.expr, cb);
	},
	"assign": function (node, cb) {
		validateJavascriptExpression(node.location, node.expr, cb);
	},
...
function validateJavascriptExpression(leftHand, rightHand, cb) {
	...
	var expression = leftHand + '=' + rightHand;
	var result = checkJavascript(expression);

	if(result.error) {
		cb(result.error);
	} else if(result.syntax.body[0].type !== 'ExpressionStatement') {
		cb('Not an expression');
	} else {
		cb();
	}
}
