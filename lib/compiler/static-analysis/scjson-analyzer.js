...
var scJsonAnalyzer = {
  analyze: function(scJson, docUrl, context, done) {
    ...
    function traverseAndAccumulateDatamodelDeclarations(state){
      if(state.datamodel) datamodelAccumulator.push.apply(datamodelAccumulator, state.datamodel);
    ...
    function checkLocationExistsInDatamodel(node){
      return datamodelAccumulator.map( data => data.id ).indexOf(node.location.expr) === -1
    }
    ...
    var actionTags = {
      ...
      'assign': function (node) {
        if(checkLocationExistsInDatamodel(node)){
          //return error 
