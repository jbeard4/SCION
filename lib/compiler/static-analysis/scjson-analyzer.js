...
var scJsonAnalyzer = {
    ...
    //init actionTags
    Object.keys(actionTags).forEach(function(k){
      actionTags['{http://www.w3.org/2005/07/scxml}' + k] = actionTags[k];
      delete actionTags[k];
    });

    actionTags['transitions'] = function (node) {
      if(node.cond) {
        // return validateJavascriptCondition(node.cond);
      ...
      return [];
		...
		function handleError (node, property, errors, $type, state, override) {
			...
			var errorNode = {
				...
				$type: '{http://www.w3.org/2005/07/scxml}raise',
		...
		function commitChanges (scJson, errors) {
			changes.forEach(function (change) {
				if(change.$type === '{http://www.w3.org/2005/07/scxml}data' && !change.new) {
					delete scJson.datamodel;
					scJson.onEntry = [ change.new || change.error ];
				} else if(change.$type === '{http://www.w3.org/2005/07/scxml}script' && !change.new && scJson.rootScripts) {
