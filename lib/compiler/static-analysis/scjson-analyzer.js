		...
		var actionTags = {
			'data': function (node, done) {
				...
				if(node.src && node.src.indexOf('file:') === 0) {
					...
					getFileContents(node.src.substring(5), function (error, content) {
						if(error) {
							...
							node.expr = { 	$column: node.$column,
											$line: node.$line,
											expr: "\"" + normalizeWhitespace(content.toString()) + "\""
										};
					...
					var errors = validateJavascriptAssignment(node.id, node.expr);

					if(errors.length > 0) {
						node.expr.expr = "\"" + normalizeWhitespace(node.expr.expr.toString()) + "\"";
