...
var scJsonAnalyzer = {
	analyze: function(scJson, docUrl, context, done) {
		...
		var actionTags = {
			...
			'log': function (node, done) {
				if(node.expr) {
					validateJavascriptExpression (node.expr, done);
				}
			},
			'send': function (node, done) {
				if(node.type) {
					validateJavascriptExpression (node.expr, done);
				}
			},
			'foreach': function (node, done) {
				if(node.item) {
					validateJavascriptExpression (node.item, done);
				}
				if(node.index) {
					validateJavascriptExpression (node.index, done);
				}
				if(node.array) {
					validateJavascriptExpression (node.array, done);
				}
			}
		};
		...
		function validateSystemVariables (js, done) {
			validateJavascriptExpression(js, function (error) {
				if(error) {
					...
					traverseSyntaxTree(syntaxTree, done);
				} 
			});
		}
		...
		var treeTypes = {
			"AssignmentExpression": function(tree, done) {
                //Check if assignee is a system variable
                if(systemVariables.indexOf(tree.expression.left.name) !== -1) {
                	done('You can\'t change system variables: ' + tree.expression.left.name);	
                } else {
                	done();
                }
            }
		};
		function traverseSyntaxTree(tree, done) {
			Object.keys(tree).forEach(function(i){
		        ...
		        if (typeof(currentNode) === 'object') {
		        	if (currentNode.type && treeTypes[currentNode.type]) {
		        		 treeTypes[currentNode.type](tree, done);
		        	}

		        	//Go deeper into the child nodes
		            traverseSyntaxTree(currentNode, done);
		        }
		    });
		}
		...
		function commitChanges (scJson) {
			changes.forEach(function (change) {
				...
				} else if(change.path.indexOf('datamodel') !== -1) {
