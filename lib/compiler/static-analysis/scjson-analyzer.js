	...
	analyze: function(scJson, docUrl, context, done) {
		...
		function processState(state) {
			...
			if(state.transitions) {
				processActions(state, 'transitions');
		...
		function processActions(actionContainer, name){
			if(Array.isArray(actionContainer[name])) {

				Object.keys(actionContainer[name]).forEach(function(i) {
					checkAction(actionContainer[name], i, actionContainer[name][i].type || name);
				});
			} else {
				checkAction(actionContainer, name, name);
			}
		}

		function checkAction(action, propertyName, type) {
		    if(actionTags[type]) {

		    	var errors = actionTags[type](action[propertyName], function (errors, override) {
		    		if(override) {
		    			handleError(action, propertyName, errors, type, override);
			    	} else if(errors.length > 0) {
						handleError(action, propertyName, errors, type);
			    	}
		    	...
		    	if(errors) {
		    		if(errors.override) {
			    		handleError(action, propertyName, errors.errors, type, errors.override);
			    	} else if(errors.length > 0) {
						handleError(action, propertyName, errors, type);
			    	}
		    	}
		...
		var actionTags = {
			...
			'foreach': function (node) {
				...
				if(node.item) {
					errors.concat(validateJavascriptExpression(node.item));
				}
				if(node.index) {
					errors.concat(validateJavascriptExpression(node.index));
				}
				if(node.array) {
					errors.concat(validateJavascriptExpression(node.array));
				}
		...
		function validateJavascriptExpression (js) {
			...
			var js = extractJavascript(js);
			var jsCheck = validateArbitraryJavascript(js);

			if(jsCheck.error) {
				errors.push(jsCheck.error);
			}
		...
		function handleError (node, property, errors, type, override) {
			var newNode, isError = true;
		...
		function commitChanges (scJson) {
			changes.forEach(function (change) {
				...
				} else if(change.type === 'script' && change.isError && scJson.rootScripts) {
