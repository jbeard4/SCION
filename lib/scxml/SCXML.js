...
SCXMLInterpreter.prototype = {
    ...
    _getStatesEntered : function(transitions) {
        ...
        var processTransitionSourceAndTarget = _.bind(function(source,target){
            //process each target
            processState(target);

            //and process ancestors of targets up to LCA, but according to special rules
            var lca = this.opts.model.getLCA(source,target);
            _.each(this.opts.model.getAncestors(target,lca),function(s){
        ...
        var processState = _.bind(function(s){
            ...
            if (s.kind === stateKinds.HISTORY) {
                ...
                if (s.kind === stateKinds.PARALLEL) {
                    _.chain(s.children).
                        filter(function(s){return s.kind !== stateKinds.HISTORY;}).
                        each(function(s){ statesToProcess.push(s); });
