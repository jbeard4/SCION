            ...
            if (!selectedTransitions.isEmpty()) {
                ...
                for (var j = 0, len = statesExited.length; j < len; j++) {
                    ...
                    if(state.onExit !== undefined) {
                        for (var exitIdx = 0, len = state.onExit.length; exitIdx < len; exitIdx++) {
                            evaluateAction(state.onExit[exitIdx]);                          
                        }
                    }
                ...
                for (var stxIdx = 0, len = sortedTransitions.length; stxIdx < len; stxIdx++) {
                    ...
                    if(transition.onTransition !== undefined) {
                        for (var txIdx = 0, txLen = transition.onTransition.length; txIdx < txLen; txIdx++) {
                            evaluateAction(transition.onTransition[txIdx]);
                        }
                    }
                }
                ...
                for (var enterIdx = 0, enterLen = statesEntered.length; enterIdx < enterLen; enterIdx++) {
                    let state = statesEntered[enterIdx];
                    ...
                    if(state.onEntry !== undefined) {
                        for (var entryIdx = 0, entryLen = state.onEntry.length; entryIdx < entryLen; entryIdx++) {
                            evaluateAction(state.onEntry[entryIdx]);                          
                        }
                    }
                }
