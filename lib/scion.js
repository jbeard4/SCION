...
(function (root, factory) {
    ...
    function Statechart(model, opts) {
        opts = opts || {};
        opts.sessionid = createSessionId();

        opts.ioprocessors = {};

        //Create all supported Event I/O processor nodes.
        //TODO fix location after implementing actual processors
        for (var processor in eventProcessorTypes) {
            opts.ioprocessors[processor] = { "location" : "http://example.com/" + eventProcessorTypes[processor] };
        }
    ...
    function createSessionId () {
        //Took from here, http://stackoverflow.com/a/23197364
        //Didn't pay huge attention to actual randomness or interference with another sessionid
    ...
    InterpreterScriptingContext.prototype = {
        ...
        send : function(event, options){
            if(!eventProcessorTypes[event.sendtype]) {
                this.raise({ name : "error.execution", data: 'Unsupported event processor type' });
            }
