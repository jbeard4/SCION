      ...
      for (var j = 0, len = statesToExit.length; j < len; j++) {
          ...
          if(stateExited.onExit !== undefined) {
              for (var exitIdx = 0, exitLen = stateExited.onExit.length; exitIdx < exitLen; exitIdx++) {
                  this._evaluateAction(null, stateExited.onExit[exitIdx]);
              }
          }
          ...
          if(state.invokes) state.invokes.forEach( invoke =>  {
            if(invoke.autoforward){
              ...
              this._scriptingContext.send({
                ...
                data : e.data,
                type: 'http://www.w3.org/TR/scxml/#SCXMLEventProcessor',
                origin: this.opts.sessionid     //TODO: preserve original origin when we autoforward? 
              });
            }
...
InterpreterScriptingContext.prototype = {
    ...
    raise : function(event){
        this._installDefaultPropsOnEvent(event);
    ...
    _installDefaultPropsOnEvent : function(event){
      [
        'name',
        'sendid',
        'origin',
    ...
    send : function(event, options){
        ...
        function defaultSendAction (event, options) {
          ...
          if(!event.target){
            ...
            //not currently supported: raise error.communication
            //TODO: add support for inter-session communication
