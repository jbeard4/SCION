...
function BaseInterpreter(modelOrFnGenerator, opts){
    ...
    opts._x = {
        ...
        _ioprocessors : opts.ioprocessors || ioProcessorTypes 
    };
...
BaseInterpreter.prototype = extend(beget(EventEmitter.prototype),{
    ...
    cancel : function(){
      ...
      console.log(`session cancelled ${this.opts.invokeid}`);
    ...
    _performBigStep : function(e) {
        ...
        this._configuration.iter().forEach(state => {
          if(state.invokes) state.invokes.forEach( invoke =>  {
            ...
            console.log('invoke.id === e.invokeId', invoke.id, e.invokeid);
        ...
        while (keepGoing) {
            ...
            keepGoing = !selectedTransitions.isEmpty() || this._internalEventQueue.length;
        }
    ...
    _evaluateAction : function(currentEvent, actionRef) {
        //console.log('evaluate action', currentEvent, actionRef);
        try {
    ...
    _getStatesEntered : function(enabledTransitions){
        ...
        this._computeEntrySet(enabledTransitions, statesToEnter, statesForDefaultEntry, defaultHistoryContent) 
    ...
    _addDescendantStatesToEnter : function(state,statesToEnter,statesForDefaultEntry, defaultHistoryContent){
      if(state.typeEnum === STATE_TYPES.HISTORY){
          if(this._historyValue[state.id]){
              ...
              defaultHistoryContent[state.parent.id] = state.transitions;
              for(let s of state.transitions[0].targets)   //FIXME: handle history transition more correctly
                  this._addDescendantStatesToEnter(s,statesToEnter,statesForDefaultEntry, defaultHistoryContent)
          ...
          if(state.typeEnum === STATE_TYPES.COMPOSITE){
              ...
              for(let s of state.initialRef)
                  this._addDescendantStatesToEnter(s,statesToEnter,statesForDefaultEntry, defaultHistoryContent)
              for(let s of state.initialRef)
                  this._addAncestorStatesToEnter(s, state, statesToEnter, statesForDefaultEntry, defaultHistoryContent)
          }else{
...
InterpreterScriptingContext.prototype = {
    ...
    invoke : function(invokeObj){
      ...
      //console.log('invokeObj', invokeObj);
      //TODO: populate idMap with Promise
      this._invokeMap[invokeObj.id] = new Promise((resolve, reject) => {
        (this._interpreter.opts.invokers || InterpreterScriptingContext.invokers)[invokeObj.type](this._interpreter, invokeObj, (err, session) => {
          ...
          resolve(session);
          //console.log('adding invoked session to _invokeMap', invokeObj.id, session);
        });
      });
    },
    cancelInvoke : function(invokeid){
      ...
      console.log(`cancelling session with invokeid ${invokeid}`);
      if(sessionPromise){
        console.log(`sessionPromise found`);
        sessionPromise.then( 
          ((session) => {
            console.log(`resolved session ${invokeid}. cancelling... `);
            session.cancel(); 
            //clean up
    ...
    _installDefaultPropsOnEvent : function(event,isInternal){
      if(!isInternal){ 
        event.origin = `#_scxml_${this._interpreter.opts.sessionid}`;     //TODO: preserve original origin when we autoforward? 
        event.origintype = event.type || ioProcessorTypes.scxml.location;
      }
    ...
    send : function(event, options){
        ...
        options = options || {};
        var sendType = options.type || ioProcessorTypes.scxml.location;
        //TODO: move these out
        function validateSend(event, options, sendAction){
          if(event.target){
            var targetIsValidUri = validateUriRegex.test(event.target)
            if(!targetIsValidUri){
              throw { name : "error.execution", data: 'Target is not valid URI', sendid: options.sendid, type : 'platform' };
            }
          }
          var eventProcessorTypes = Object.keys(ioProcessorTypes).map(function(k){return ioProcessorTypes[k].location});
          if( eventProcessorTypes.indexOf(sendType) === -1) {
              throw { name : "error.execution", data: 'Unsupported event processor type', sendid: options.sendid, type : 'platform' };
          }
        ...
        function defaultSendAction (event, options) {
          ...
          if(event.target === '#_internal'){
            ...
            event.origintype = ioProcessorTypes.scxml.location;   //TODO : paramterize this based on send/@type?
            if(!event.target){
              ...
              if(this._interpreter.opts.parentSession){
                ...
                console.log('event.invokeid', event.invokeid)
                doSend.call(this, this._interpreter.opts.parentSession);
              }else{
                throw { name : "error.communication", data: 'Parent session not specified', sendid: options.sendid, type : 'platform' };
              }
              ...
              if(session){
                ...
                throw {name : 'error.communication', sendid: options.sendid, type : 'platform'};
              }
            }else if(match = event.target.match(this.invokeSendTargetRegex)){
              //TODO: test this code path.
              var invokeId = match[1]
              console.log('this._invokeMap[invokeId]', invokeId, this._invokeMap);
          ...
          function doSend(session){
            ...
            var timeoutHandle = setTimeout(function(){
              if (options.sendid) delete this._timeoutMap[options.sendid];
            ...
            if (options.sendid) this._timeoutMap[options.sendid] = timeoutHandle;
