...
class SCJSONToKGraphTransformer extends EventEmitter{
  ...
  transform(scjson){
    ...
    return [klayNodeToScjsonMap, this._scjsonStateToKlayNode(klayNodeToScjsonMap, scjson, scjson, scjson)];
  }
  ...
  _scjsonStateToKlayNode(klayNodeToScjsonMap, rootState, parentState, state){
    ...
    events.node.forEach(function(eventName){
      stateKlayNode.on('node:' + eventName, function(domEvent){
        ...
        var rootScjsonNode = klayNodeToScjsonMap[rootState];
        rootScjsonNode.emit('node:' + eventName, state, stateKlayNode, domEvent);
      }.bind(this));
    }, this);
