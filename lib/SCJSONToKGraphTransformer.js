...
SCJSONToKGraphTransformer.prototype = {
  ...
  _scjsonStateToKlayNode : function (parentState, state){
    ...
    if(state.$type !== state._klayNode.$type){
      state._klayNode.$type = state.$type;  //copy in type information
    }
    if(state.transitions){
      var transitionsToInit = state.transitions.filter(function(transition){return !transition._klayEdge;});
      var newEdges = 
        transitionsToInit
      ...
      parentState._klayNode.edges.push.apply(parentState._klayNode.edges, newEdges);
    }
    if(state.states){
