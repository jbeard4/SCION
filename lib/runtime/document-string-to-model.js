...
function documentStringToModel(url,docString,cb,hostContext){
    ...
    if(
        (util.IS_INSPECTING                //workaround for node --inspect bug: https://github.com/nodejs/node/issues/8369
...
SCModel.prototype.prepare = function(cb, executionContext, hostContext) {
    ...
    for(let compiledInvokeConstructor of this.compiledInvokeConstructors){
      ...
      Promise.all(scriptPromises).then(function resolved(scripts) {
          try {
              if (compiledInvokeConstructor.datamodel) {
                  compiledInvokeConstructor.datamodel.runInContext(executionContext);
              }
              ...
              for (let i = 0; i < compiledInvokeConstructor.rootScripts.length; i++) {
                  ...
                  scriptInfo.compiled.runInContext(executionContext);
              }

              let modelFn = compiledInvokeConstructor.module.runInContext(executionContext);   //each iteration, add invokers to the execution context, which is shared memory
              executionContext[modelFn.name] = modelFn; //this should allow each FnModel to reference every other by name
              console.log('executionContext', Object.keys(executionContext));
