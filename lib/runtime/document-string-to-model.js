...
SCModel.prototype.prepare = function(cb, executionContext, hostContext) {
    ...
    Promise.all(scriptPromises).then(function resolved(scripts) {
        try {
            ...
            for (let i = 0; i < self.rootScripts.length; i++) {
                var scriptInfo = self.rootScripts[i];
                if(scriptInfo.src){
...
function createModule(url,scJson,hostContext,cb){
    ...
    scjsonToModule.startTraversal(url, scJson, hostContext).then(function resolved(rawModule) {
        ...
        for (var i = 0; i < rawModule.rootScripts.length; i++) {
            var curScript = rawModule.rootScripts[i];
            if (curScript.src) {
