...
SVGRenderer.prototype = {
  ...
  _initDefs : function(svg){
    ...
    radialGradient.setAttributeNS(null,'id',constants.finalStateGradientId);
    ...
    [
      ...
      ["stop-color:#000000;stop-opacity:1;","0.85576922"],
      ["stop-color:#ffffff;stop-opacity:1;","0.85576922"],
      ["stop-color:#ffffff;stop-opacity:1;","1"]
    ].forEach(function(params){
      var stop = document.createElementNS(SVGNS,'stop'); 
      stop.setAttributeNS(null, 'style', params[0]); 
      stop.setAttributeNS(null, 'offset', params[1]); 
  ...
  _renderGraphNode : function(parentGraphNode, graphNode){
    ...
    if(!this._kgraphNodeToSVGNode.has(graphNode)){
      ...
      //update type class
      ['history','initial','parallel','final'].forEach(function(type){
        displayNode.removeClass(type);
      });
      if(graphNode.$type){
        displayNode.addClass(graphNode.$type);
      }
