...
SVGRenderer.prototype = {
  ...
  _renderGraphNode : function(parentGraphNode, graphNode){
    ...
    if(!this._kgraphNodeToSVGNode.has(graphNode)){
      ...
      if(graphNode.$type === 'initial'){
        var circle = group.circle(graphNode.width/2, graphNode.width/2, 0);

        //start animations
        circle.animate({r : graphNode.width/2}, constants.ANIM_DURATION, mina.bounce);

        group.exit = function(){
          circle.animate({r: 0}, constants.ANIM_DURATION, mina.easein, function(){
            group.remove();
          });
        };
      }else{
        rect = group.rect(graphNode.width / 2, graphNode.height / 2, 0, 0).attr({rx : 2, ry : 2, strokeWidth : 0});
        label = group.text.apply(group, labelTextParams).attr({opacity : 0});
