...
SVGRenderer.prototype = {
  ...
  render : function(graphRoot){
    ...
    if(this._firstRender){
      this._cachedWidth = graphRoot.width;
      this._cachedHeight = graphRoot.height;
      this._s.attr('viewBox','0 0 ' + graphRoot.width + ' ' + graphRoot.height);
      this._firstRender = false;

      this._animationElement = document.createElementNS(SVGNS, 'animate');
      this._animationElement.setAttribute('attributeName','viewBox'); 
      this._animationElement.setAttribute('begin','0s'); 
      this._animationElement.setAttribute('dur',constants.ANIM_DURATION + 'ms');
      this._animationElement.setAttribute('fill','freeze');
      this._s.node.appendChild(this._animationElement);
    } else{
      // workaround, generate an animation fragment (to test browser support)
      this._animationElement.setAttribute('values', '0 0 ' + this._cachedWidth  + ' ' + this._cachedHeight + '; 0 0 ' + graphRoot.width + ' ' + graphRoot.height);
      this._animationElement.beginElement();
    }
    this._cachedWidth = graphRoot.width;
    this._cachedHeight = graphRoot.height;
  ...
  _exitKGraphObject : function(kgraphNode, property, recursiveDelete){
    ...
    var nodesToExit = recursiveDelete ? kgraphNode[property] || [] : _.difference(cache.get(kgraphNode) || [], kgraphNode[property]);
  ...
  _renderGraphNode : function(parentGraphNode, graphNode){
    ...
    if(!this._kgraphNodeToSVGNode.has(graphNode)){
      ...
      if(graphNode.$type){
        displayNode.addClass(graphNode.$type);
      }else{
        displayNode.removeClass('parallel').removeClass('history');
      }
  ...
  _renderEdge : function(parentGraphNode, edge){
    ...
    if(!this._kgraphNodeToSVGNode.has(edge)){
      ...
      edgeDisplayNode.animate({d : d}, constants.ANIM_DURATION);    //TODO: support vertical layout
