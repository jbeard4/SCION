    ...
    _performSmallStep : function(eventSet, datamodelForNextStep) {
        ...
        if (!selectedTransitions.isEmpty()) {
            ...
            for (var key in datamodelForNextStep) {
                if (printTrace) platform.log("key ", key);

                if (key in this._datamodel) {
                    if (printTrace) platform.log("old value ", this._datamodel[key]);
                } else {
                    if (printTrace) platform.log("old value is null");
                }
                if (printTrace) platform.log("new value ", datamodelForNextStep[key]);
