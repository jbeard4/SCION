...
SCXMLInterpreter.prototype = {
    ...
    start : function() {
        ...
        if (this.opts.printTrace) platform.log("performing initial big step");
    ...
    _performSmallStep : function(eventSet, datamodelForNextStep) {
        ...
        if (this.opts.printTrace) platform.log("selecting transitions with eventSet: ", eventSet);

        var selectedTransitions = this._selectTransitions(eventSet, datamodelForNextStep);

        if (this.opts.printTrace) platform.log("selected transitions: ", selectedTransitions);

        if (!selectedTransitions.isEmpty()) {

            if (this.opts.printTrace) platform.log("sorted transitions: ", selectedTransitions);
            ...
            if (this.opts.printTrace) platform.log("basicStatesExited ", basicStatesExited);
            if (this.opts.printTrace) platform.log("basicStatesEntered ", basicStatesEntered);
            if (this.opts.printTrace) platform.log("statesExited ", statesExited);
            if (this.opts.printTrace) platform.log("statesEntered ", statesEntered);

            var eventsToAddToInnerQueue = new this.opts.EventSet();

            //update history states
            if (this.opts.printTrace) platform.log("executing state exit actions");

            statesExited.forEach(function(state){

                if (this.opts.printTrace) platform.log("exiting ", state);
            ...
            if (this.opts.printTrace) platform.log("executing transitition actions");
            ...
            if (this.opts.printTrace) platform.log("executing state enter actions");
            ...
            if (this.opts.printTrace) platform.log("updating configuration ");
            if (this.opts.printTrace) platform.log("old configuration ", this._configuration);

            //update configuration by removing basic states exited, and adding basic states entered
            this._configuration.difference(basicStatesExited);
            this._configuration.union(basicStatesEntered);

            if (this.opts.printTrace) platform.log("new configuration ", this._configuration);
            
            //add set of generated events to the innerEventQueue -> Event Lifelines: Next small-step
            if (!eventsToAddToInnerQueue.isEmpty()) {
                if (this.opts.printTrace) platform.log("adding triggered events to inner queue ", eventsToAddToInnerQueue);
                this._innerEventQueue.push(eventsToAddToInnerQueue);
            }

            if (this.opts.printTrace) platform.log("updating datamodel for next small step :");
            
            //update the datamodel
            for (var key in datamodelForNextStep) {
                if (this.opts.printTrace) platform.log("key ", key);

                if (key in this._datamodel) {
                    if (this.opts.printTrace) platform.log("old value ", this._datamodel[key]);
                } else {
                    if (this.opts.printTrace) platform.log("old value is null");
                }
                if (this.opts.printTrace) platform.log("new value ", datamodelForNextStep[key]);
    ...
    _selectTransitions : function(eventSet, datamodelForNextStep) {
        ...
        if (this.opts.printTrace) platform.log("priorityEnabledTransitions", priorityEnabledTransitions);
    ...
    _selectPriorityEnabledTransitions : function(enabledTransitions) {
        ...
        if (this.opts.printTrace) platform.log("enabledTransitions", enabledTransitions);
        if (this.opts.printTrace) platform.log("consistentTransitions", consistentTransitions);
        if (this.opts.printTrace) platform.log("inconsistentTransitionsPairs", inconsistentTransitionsPairs);
        if (this.opts.printTrace) platform.log("priorityEnabledTransitions", priorityEnabledTransitions);
    ...
    _getInconsistentTransitions : function(transitions) {
        ...
        if (this.opts.printTrace) platform.log("transitions", transitionList);
