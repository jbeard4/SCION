...
function SCXMLInterpreter(model, opts){
    ...
    this.opts.log = this.opts.log || (platform && platform.log);   //rely on global console if this console is undefined
...
SimpleInterpreter.prototype._send = function(event, options) {
    var callback, timeoutId,
        _this = this;
    if (platform && platform.setTimeout) {
        if (printTrace) {
...
SimpleInterpreter.prototype._cancel = function(sendid){
    if (platform && platform.clearTimeout) {
