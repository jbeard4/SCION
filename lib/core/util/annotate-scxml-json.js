...
var codeGen = require('./action-code-generator'),
...
var SCXML_NS = "http://www.w3.org/2005/07/scxml";
...
function genRootScripts(root) {
    return dom.filter(root,function(c){c.localName === "script";}).map(function(c){return c.textContent;});
}
...
function transformTransitionNode (transitionNode, parentState) {
    ...
    if (transitionNode.hasAttribute('event')) {
        ...
        var event = transitionNode.getAttribute('event');
        if (event === "*") {
            events = [event];
        } else {
    ...
    var transition = {
        ...
        cond: transitionNode.getAttribute("cond"),
        events: events,
        targets: transitionNode.hasAttribute("target") ? transitionNode.getAttribute("target").trim().split(/\s+/) : null
    };

    if(dom.getElementChildren(transitionNode).length) transition.actions = codeGen.generateCode(transitionNode);
...
function transformDatamodel(node, ancestors) {
    dom.filter(node,function(child){return child.localName === 'data';}).forEach(function(child){
        if (child.hasAttribute("id")) {
            datamodel[child.getAttribute("id")] = child.hasAttribute("expr") ? child.getAttribute("expr") : null;
        }
    });
}

function transformStateNode(node, ancestors) {
    var id = node.hasAttribute("id") ? node.getAttribute("id") :  genId(node.localName);
    var kind; 

    switch (node.localName) {
        case "state":
            if( dom.filter(node,function(child){return STATE_TAGS.indexOf(child.localName) > -1;}).length){
    ...
    if (kind === stateKinds.HISTORY) {
        state.isDeep = node.getAttribute("type") === "deep" ? true : false;
    }
    ...
    dom.getElementChildren(node).forEach(function(child){
        ...
        switch (child.localName) {
            case "transition":
                ...
                break;
            case "onentry":
                onEntryChildren = codeGen.generateCode(child);
                break;
            case "onexit":
                onExitChildren = codeGen.generateCode(child);
                break;
            case "initial":
                if (!processedInitial) {
            ...
            case "datamodel":
                ...
                break;
            default:
                if(STATES_THAT_CAN_BE_CHILDREN.indexOf(child.localName) > -1){
    ...
    if (!processedInitial && node.localName !== "parallel") {
        ...
        function generateFakeInitialState(targetId) {
            var initial = document.createElementNS(SCXML_NS,"initial");
            var transition = document.createElementNS(SCXML_NS,"transition");
        ...
        if (hasInitialAttribute) {
            generateFakeInitialState(node.getAttribute("initial"));
        } else {
