...
require ["scxml/json2model","scxml/json2extra-model", "scxml/test/harness", "scxml/async-for","util/set/ArraySet","util/set/BitVector","util/set/BooleanArray","util/set/ObjectSet", "spartanLoaderForAllTests", "class-transition-lookup-optimization-loader", "switch-transition-lookup-optimization-loader", "table-transition-lookup-optimization-loader","scxml/model","scxml/extra-model"],(json2model,json2ExtraModel,harness,asyncForEach,ArraySet,BitVectorInitializer,BooleanArrayInitializer,ObjectSetInitializer,testTuples,classTransitionOpts,switchTransitionOpts,tableTransitionOpts,m,extraModel) ->
	...
	model = json2model args.scxmlJson
	...
	optArg =
		switch optName
			when "default transition lookup" then {}
			when "class-transition-lookup" then {transitionSelector:classTransOpt,onlySelectFromBasicStates:true}
			when "switch-transition-lookup" then {transitionSelector:switchTransOpt}
			when "table-transition-lookup" then {transitionSelector:tableTransOpt}
